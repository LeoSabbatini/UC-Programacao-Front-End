...
3. Escreva um programa que declara um objeto carro, com as propriedades `nome (string)`, `cor (string)`, `modelo (string)` e `opcionais (object)`, contendo `ar condicionado (boolean)`, `direção elétrica (boolean)` e `multimídia (boolean)`.
